                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <table id="file_export" class="table table-striped table-bordered display">
                      <thead>
                        <tr>
						 <th>Sl.no</th>
                          <th>Title</th>
						 <th>Content</th>
                          <th>Silver Club Achivers Image</th>
                          <th>Date</th>
                          <th>Action</th>
                        </tr>
                      </thead>
					  ...
					  <?php 
						$i=1;
						$sql="SELECT * from tbl_add_silver_club_achivers_meet "; 
                        ...
                        <tr>
                          ...
                          <td><?php echo $row['title'];?></td>
						   ...
						   <td  class="pro-list-img"><img src="<?php echo $row['silver_club_achivers_image'];?>" style="height: 44px;"></td>
                  ...
                  <div id="edit-contact<?php echo $row['id'];?>" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                    ...
                    <?php 
						$id=$row['id'];
						$exe="SELECT * FROM tbl_add_silver_club_achivers_meet where id='$id' ";
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <img src="<?php echo $row['silver_club_achivers_image'];?>" alt="image"  width="100px" height="100px">
                      </div>
                      <div class="form-group">
                        <input id="silver_club_achivers_image" class="form-control" name="img_files" type="file">
                      </div>
                      
							  <input type="hidden" value="<?php echo $row['id'];?>" name="id">
							  <input type="hidden" value="<?php echo $row['silver_club_achivers_image'];?>" name="image" >
                            </div>                         
                        </div>
				  ...
				  <div class="modal-dialog modal-lg" role="document">
					...
					<?php 
						$id=$row['id'];
						$execution="SELECT * FROM tbl_add_silver_club_achivers_meet where id='$id' ";
						$exe_results=mysqli_query($con,$execution);
						$rowresults = mysqli_fetch_array($exe_results);
						?>
					  <form method="post">
					  <input type="hidden" value="<?php echo $rowresult['silver_club_achivers_image'];?>" name="image" >
					  <div class="modal-body">
                      ...
                      <div class="modal-content">
                        ...
                        <div class="modal-body">                         
							...
							<div class="col-md-12 mb-3">
											...
											<input type="file" class="form-control" id="validationCustom03" required name="silver_club_achivers_image">
										</div>
										</div>
                        <div class="modal-footer">
...
if(isset($_POST['submit'] ) ) {
  ...
  $filename = $_FILES["silver_club_achivers_image"]["name"];
      ...
      // Validate file type
      if (in_array($_FILES["silver_club_achivers_image"]["type"], $allowed_types)) {
          // Validate file size
          if ($_FILES["silver_club_achivers_image"]["size"] <= $max_size) {
              $upload_dir = "assets/images/gallery/";
              $file_name = basename($_FILES["silver_club_achivers_image"]["name"]);
              $destination = $upload_dir . $file_name;
            
              if (move_uploaded_file($_FILES["silver_club_achivers_image"]["tmp_name"], $destination)) {
                try {
       ...
       $sql_query ="INSERT INTO tbl_add_silver_club_achivers_meet (title,content,silver_club_achivers_image,registered_date)VALUES ('$title','$content','$cloudinaryUrl','$registered_date')";
              $query = mysqli_query($con,$sql_query); 

              if ($query) {
                echo '<script type="text/javascript">alert("Insert Successfully"); window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
            } else {
                echo '<script type="text/javascript">alert("Failed to insert into database"); window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
            }
        } catch (Exception $e) {
            echo 'Cloudinary upload failed: ' . $e->getMessage();
        }
    } else {
        echo '<script type="text/javascript">alert("Failed to move uploaded file"); window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
    }
}
//  else {
//     echo '<script type="text/javascript">alert("File size exceeds the limit of 5 MB"); window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
// }
} else {
echo '<script type="text/javascript">alert("Invalid file type"); window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
}
}
?>
...
{
		$id=$_POST['id'];
		$silver_club_achivers_image =$_POST['silver_club_achivers_image'];
	 unlink("assets/images/gallery/".$silver_club_achivers_image);
	 unlink("assets/images/gallery/thumb/".$silver_club_achivers_image);
		$sql="DELETE FROM  tbl_add_silver_club_achivers_meet WHERE id='$id'";
		$res=mysqli_query($con,$sql);
		if($res){
			echo '<script type="text/javascript">alert("Deleted sucessfully");window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
...
if (isset($_POST['update'])) {
    ...
    if (!empty($file_name)) {
        ...
        if (!in_array($image_mime, $valid_image_check)) {
            echo '<script type="text/javascript">alert("Invalid image format.");window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
            exit();
        }

        // Move uploaded file
        if (!move_uploaded_file($_FILES["img_files"]["tmp_name"], $filepath)) {
            echo '<script type="text/javascript">alert("Failed to upload ' . $_FILES["img_files"]["name"] . '");window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
            exit();
        }
        ...
        try {
            ...
            $sql = "UPDATE `tbl_add_silver_club_achivers_meet` SET title='$title', content='$content', silver_club_achivers_image='$cloudinaryUrl' WHERE id='$id'";
        } catch (Exception $e) {
            echo 'Cloudinary upload failed: ' . $e->getMessage();
            exit();
        }
        ...
        $sql = "UPDATE `tbl_add_silver_club_achivers_meet` SET title='$title', content='$content' WHERE id='$id'";
    }

    $result = mysqli_query($con, $sql);
    if ($result) {
        echo '<script type="text/javascript">alert("Updated successfully.");window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
    } else {
        echo '<script type="text/javascript">alert("Failed to update.");window.location.href = "view_Silver_Club_Achivers_meet.php";</script>';
    }
}
?>
