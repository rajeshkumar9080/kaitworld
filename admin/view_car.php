                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <table id="file_export" class="table table-striped table-bordered display">
					  ...
					  <?php 
						$i=1;
						$sql="SELECT * from tbl_add_car_achivers "; 
                        ...
                        <tr>
                          ...
                          <td><?php echo $row['title'];?></td>
						   ...
						   <td  class="pro-list-img"><img src="<?php echo $row['car_achivers_image'];?>" style="height: 44px;"></td>
                  ...
                  <div id="edit-contact<?php echo $row['id'];?>" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                    ...
                    <?php 
						$id=$row['id'];
						$exe="SELECT * FROM tbl_add_car_achivers where id='$id' ";
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <img src="<?php echo $row['car_achivers_image'];?>" alt="Mobile image"  width="100px" height="100px">
                      </div>
                      <div class="form-group">
                        <input id="image" class="form-control" name="img_files" type="file">
							  ...
							  <input type="hidden" value="<?php echo $row['car_achivers_image'];?>" name="car_achivers_image" >
                            </div>                         
                        </div>
				  ...
				  <div class="modal-dialog modal-lg" role="document">
					...
					<?php 
						$id=$row['id'];
						$execution="SELECT * FROM tbl_add_car_achivers where id='$id' ";
						$exe_results=mysqli_query($con,$execution);
						$rowresults = mysqli_fetch_array($exe_results);        
						?>
					  <form method="post">
					  <input type="hidden" value="<?php echo $rowresult['car_achivers_image'];?>" name="car_achivers_image" >    
					  <div class="modal-body">
                      ...
                      <div class="modal-content">
                        ...
                        <div class="modal-body">                         
							...
							<div class="col-md-12 mb-3">
											...
											<input type="file" class="form-control" id="validationCustom03" required name="car_achivers_image">
										</div>
										</div>
                        <div class="modal-footer">
...
if (isset($_POST['submit'])) {
  ...
  $filename = $_FILES["car_achivers_image"]["name"];
  
  $allowed_types = array("image/jpeg", "image/png", "image/gif", "image/bmp", "image/gif");
  $max_size = 5 * 1024 * 1024; // 5 MB
  
  // Validate file type
  if (in_array($_FILES["car_achivers_image"]["type"], $allowed_types)) {
      // Validate file size
      if ($_FILES["car_achivers_image"]["size"] <= $max_size) {
          // Move the uploaded file to the desired directory
          $upload_dir = "assets/images/gallery/";
          $file_name = basename($_FILES["car_achivers_image"]["name"]);
          $destination = $upload_dir . $file_name;
          
          if (move_uploaded_file($_FILES["car_achivers_image"]["tmp_name"], $destination)) {
              ...
              try {
                  ...
                  $sql_query = "INSERT INTO tbl_add_car_achivers (title, content, car_achivers_image, registered_date) VALUES ('$title', '$content', '$cloudinaryUrl', '$registered_date')";
                  $query = mysqli_query($con, $sql_query);
                  
                  if ($query) {
                      echo '<script type="text/javascript">alert("Insert Successfully"); window.location.href = "view_car.php";</script>';
                  } else {
                      echo '<script type="text/javascript">alert("Failed to insert into database"); window.location.href = "view_car.php";</script>';
                  }
              } catch (Exception $e) {
                  echo 'Cloudinary upload failed: ' . $e->getMessage();
              }
          } else {
              echo '<script type="text/javascript">alert("Failed to move uploaded file"); window.location.href = "view_car.php";</script>';
          }
      }
      //  else {
      //     echo '<script type="text/javascript">alert("File size exceeds the limit of 5 MB"); window.location.href = "view_car.php";</script>';
      // }
  } else {
      echo '<script type="text/javascript">alert("Invalid file type"); window.location.href = "view_car.php";</script>';
  }
}
?>
...
{
		$id=$_POST['id'];
		$car_achivers_image=$_POST['car_achivers_image'];
	 unlink("assets/images/gallery/".$car_achivers_image);
	 unlink("assets/images/gallery/thumb/".$car_achivers_image);
		$sql="DELETE FROM tbl_add_car_achivers WHERE id='$id'";
		$res=mysqli_query($con,$sql);
		if($res){
			echo '<script type="text/javascript">alert("Deleted sucessfully");window.location.href = "view_car.php";</script>';
    ...
    $car_achivers_image = mysqli_real_escape_string($con, $_POST['car_achivers_image']);
    ...
    if (!empty($file_name)) {
        ...
        if (!in_array($image_mime, $valid_image_check)) {
            echo '<script type="text/javascript">alert("Invalid image format.");window.location.href = "view_car.php";</script>';
            // exit();
        }

        // Move uploaded file to temporary location
        if (!move_uploaded_file($_FILES["img_files"]["tmp_name"], $filepath)) {
            echo '<script type="text/javascript">alert("Failed to upload ' . $_FILES["img_files"]["name"] . '");window.location.href = "view_car.php";</script>';
            // exit();
        }
        // Upload the image to Cloudinary
        try {
            ...
            $sql = "UPDATE `tbl_add_car_achivers` SET title='$title', content='$content', car_achivers_image='$cloudinaryUrl' WHERE id='$id'";
        } catch (Exception $e) {
            echo 'Cloudinary upload failed: ' . $e->getMessage();
            exit();
        }
        ...
        $sql = "UPDATE `tbl_add_car_achivers` SET title='$title', content='$content' WHERE id='$id'";
    }

    $result = mysqli_query($con, $sql);
    if ($result) {
        echo '<script type="text/javascript">alert("Updated successfully.");window.location.href = "view_car.php";</script>';
    } else {
        echo '<script type="text/javascript">alert("Failed to update.");window.location.href = "view_car.php";</script>';
    }
}
?>
<script>
