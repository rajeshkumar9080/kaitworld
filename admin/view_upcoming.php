                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <table id="file_export" class="table table-striped table-bordered display">
					  ...
					  <?php 
						$i=1;
						$sql="select * from  tbl_add_upcoming "; 
                        ...
                        <tr>
						  <td><?php echo $i++;?></td>
                          <td><?php echo $row['up_title1'];?></td>
						    <td><?php echo $row['up_title2'];?></td>
						  <td><?php echo $row['up_content']; ?></td>
						   <td  class="pro-list-img"><img src="assets/images/gallery/<?php echo $row['up_image'];?>" style="height: 44px;"></td>
                  ...
                  <div id="edit-contact<?php echo $row['user_id'];?>" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
					...
					<?php 
						$user_id=$row['user_id'];
						$exe="	SELECT * FROM  tbl_add_upcoming where  user_id='$user_id' ";
                        ...
                        <div class="modal-body">                                          
                            <div class="form-group">
                              <div class="col-md-12 mb-3">
                                <input type="text" class="form-control" name="up_title1" required placeholder="Country name" value="<?php echo $rowresult['up_title1'];?>"/>
                              </div> 
							  <div class="col-md-12 mb-3">
							    <input type="text" class="form-control" name="up_title2" required placeholder="Country name" value="<?php echo $row['up_title2'];?>"/>
                              </div> 
							   <div class="col-md-12 mb-3">
							   <textarea id="content_page" class="form-control editor" name="up_content" type="text"><?php echo $row['up_content'];?></textarea>
                           
                              </div>
							  <div class="col-md-12 mb-3">
                         <label for="image">Photo</label>
                        <img src="assets/images/gallery/<?php echo $row['up_image'];?>" alt="image"  width="100px" height="100px">
                      </div>
                      <div class="form-group">
                        <input id="image" class="form-control" name="img_files" type="file">
							  ...
							  <input type="hidden" value="<?php echo $row['up_image'];?>" name="up_image" >
                            </div>                         
                        </div>
				  ...
				  <div class="modal-dialog modal-lg" role="document">
					...
					<?php 
						$user_id=$row['user_id'];
						$execution="	SELECT * FROM   tbl_add_upcoming where  user_id='$user_id' ";
						$exe_results=mysqli_query($con,$execution);
						$rowresults = mysqli_fetch_array($exe_results);
						?>
					  <form method="post">
					  <input type="hidden" value="<?php echo $rowresult['up_image'];?>" name="up_image" >
					  <div class="modal-body">
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">                         
                            <div class="form-group">
                              <div class="col-md-12 mb-3">
                                <input type="text" class="form-control" name="up_title1" required placeholder="Title"/>
                              </div>
							     <div class="col-md-12 mb-3">
                                <input type="text" class="form-control" name="up_title2" required placeholder="Title"/>
                              </div>
                            </div>
							<div class="form-group">
                              <div class="col-md-12 mb-3">
							   <textarea id="content_page" class="form-control editor" required name="up_content" type="text" ></textarea>
							...
							<div class="col-md-12 mb-3">
											...
											<input type="file" class="form-control" id="validationCustom03" required name="up_image">
										</div>
										</div>
                        <div class="modal-footer">
...
if( isset($_POST['save_user'] ) ) {
	$up_title1=$_POST['up_title1'];
	$up_title2=$_POST['up_title2'];
 $up_content=$_POST['up_content'];
 $filename = $_FILES["up_image"]["name"];
  $registered_date= date('Y-m-d'); 
	  //  require_once('./php-image-magician/php_image_magician.php');
  $valid_image_check = array("image/gif", "image/jpeg", "image/jpg", "image/png", "image/bmp");
  if (count($_FILES["up_image"]) > 0) {
    $folderName = "assets/images/gallery/";
      if ($_FILES["up_image"]["name"] <> "") {

        $image_mime = strtolower(image_type_to_mime_type(exif_imagetype($_FILES["up_image"]["tmp_name"])));
		//size set
		$image_info = getimagesize($_FILES["up_image"]["tmp_name"]);
		...
		if ($image_width >= 480 || $image_height >= 260)
       {
        ...
        if (in_array($image_mime, $valid_image_check)) {
          ...
          if (!move_uploaded_file($_FILES["up_image"]["tmp_name"], $filepath)) {
            $emsg .= "Failed to upload <strong>" . $_FILES["up_image"]["name"] . "</strong>. <br>";
            $counter++;
          } else {
       ...
       echo $sql_query ="INSERT INTO tbl_add_upcoming (up_title1,up_title2,up_content,up_image,registered_date)VALUES ('$up_title1','$up_title2','$up_content','$filename','$registered_date')";
              $query = mysqli_query($con,$sql_query); 
              // $result = mysqli_num_rows($query);
              if ($query) {
                // file uplaoded successfully.
				echo '<script type="text/javascript">alert("Insert Sucessfully");
               window.location.href = "view_upcoming.php";</script>';
              }else{
...
if ($_POST["delete"])
{
	 ...
	 unlink("assets/images/gallery/".$up_image);
	 unlink("assets/images/gallery/thumb/".$up_image);
		$sql=" DELETE FROM   tbl_add_upcoming  WHERE user_id='$cid'";
		$res=mysqli_query($con,$sql);
		if($res){
			echo '<script type="text/javascript">alert("Deleted sucessfully");window.location.href = "view_upcoming.php";</script>';
...
$file_name = $_FILES["img_files"]["name"];
 $images=$_POST['images'];
  if( isset($file_name) && $file_name == "") {
	echo $sql = ' UPDATE  tbl_add_upcoming SET up_title1="'.$up_title1.'",up_title2="'.$up_title2.'",up_content="'.$up_content.'" WHERE user_id="'.$user_id.'"';
	$result = mysqli_query($con,$sql);
	 						 
		 echo '<script type="text/javascript">alert("Updated sucessfully");window.location.href = "view_upcoming.php";</script>';
		...
		if ($image_width >= 360 || $image_height >= 280)
       {
        if (in_array($image_mime, $valid_image_check)) {
          ...
          if (!move_uploaded_file($_FILES["img_files"]["tmp_name"], $filepath)) {
            ...
            unlink("assets/images/gallery/".$up_image);
			unlink("assets/images/gallery/thumb/".$up_image);
 echo $sql =' UPDATE  tbl_add_upcoming SET up_title1="'.$up_title1.'",up_title2="'.$up_title2.'",up_content="'.$up_content.'",up_image="'.$filename.'"  WHERE user_id="'.$user_id.'"';
			$result = mysqli_query($con,$sql);
              if ($result) {
                // file uplaoded successfully.
				echo '<script type="text/javascript">alert("uploaded successfully.");window.location.href = "view_upcoming.php";</script>';
              } else {
                // failed to insert into database.
				echo '<script type="text/javascript">alert("Failed to upload ' . $filename . '");window.location.href = "view_upcoming.php";</script>';
              }
            
            /*             * ****** insert into database ends ******** */
          }
        } else {
		   echo '<script type="text/javascript">alert("<strong>' . $_FILES["img_files"]["name"] . '</strong>not a valid image.");window.location.href = "view_upcoming.php";</script>';
        }
		}else{
		   echo '<script type="text/javascript">alert("'.$_FILES["img_files"]["name"].' image not upload because width 360 X 280 px height above image only upload ");window.location.href = "view_upcoming.php";
               </script>';  exit();
	   }
      }
