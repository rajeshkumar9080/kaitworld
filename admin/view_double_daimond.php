                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <table id="file_export" class="table table-striped table-bordered display">
					  ...
					  <?php  
						$i=1;
						$sql="SELECT * from tbl_add_double_daimond"; 
                  ...
                  <div id="edit-contact<?php echo $row['id'];?>" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
					...
					<?php 
						$id=$row['id'];
						$exe="SELECT * FROM tbl_add_double_daimond where id='$id' ";
				  ...
				  <div class="modal-dialog modal-lg" role="document">
					...
					<?php 
						$id=$row['id'];
						$execution="	SELECT * FROM  tbl_add_double_daimond where id='$id' ";
  ...
  $filename = $_FILES["user_image"]["name"];
                    ...
                    if (in_array($_FILES["user_image"]["type"], $allowed_types)) {
                        ...
                        if ($_FILES["user_image"]["size"] <= $max_size) {
                            ...
                            echo  $sql_query ="INSERT INTO  tbl_add_double_daimond (user_id,user_name,user_image,registered_date)VALUES ('$user_id','$user_name','$cloudinaryUrl','$registered_date')";
                            $query = mysqli_query($con, $sql_query);                              // $result = mysqli_num_rows($query);
                              if ($query) {
                                echo '<script type="text/javascript">alert("Insert Successfully"); window.location.href = "view_double_daimond.php";</script>';
                            } else {
                                echo '<script type="text/javascript">alert("Failed to insert into database"); window.location.href = "view_double_daimond.php";</script>';
                            }
                        } catch (Exception $e) {
                            echo 'Cloudinary upload failed: ' . $e->getMessage();
                        }
                    } else {
                        echo '<script type="text/javascript">alert("Failed to move uploaded file"); window.location.href = "view_double_daimond.php";</script>';
                    }
                }
                //  else {
                //     echo '<script type="text/javascript">alert("File size exceeds the limit of 5 MB"); window.location.href = "view_double_daimond.php";</script>';
                // }
            } else {
                echo '<script type="text/javascript">alert("Invalid file type"); window.location.href = "view_double_daimond.php";</script>';
            }
          }
          ?>
...
if ($_POST["delete"])
{
	 ...
	 unlink("assets/images/gallery/thumb/".$user_image);
		$sql="DELETE FROM  tbl_add_double_daimond  WHERE id='$id'";
		$res=mysqli_query($con,$sql);
		if($res){
			echo '<script type="text/javascript">alert("Deleted sucessfully");window.location.href = "view_double_daimond.php";</script>';
    ...
    if (!empty($file_name)) {
        ...
        if (!in_array($image_mime, $valid_image_check)) {
            echo '<script type="text/javascript">alert("Invalid image format.");window.location.href = "view_double_daimond.php";</script>';
            // exit();
        }

        // Move uploaded file
        if (!move_uploaded_file($_FILES["img_files"]["tmp_name"], $filepath)) {
            echo '<script type="text/javascript">alert("Failed to upload ' . $_FILES["img_files"]["name"] . '");window.location.href = "view_double_daimond.php";</script>';
            // exit();
        }
          try {
            ...
            // Update database with the new image file
            $sql = "UPDATE tbl_add_double_daimond SET user_id='$user_id', user_name='$user_name', user_image='$cloudinaryUrl' WHERE id='$id'";
        } catch (Exception $e) {
            echo 'Cloudinary upload failed: ' . $e->getMessage();
            exit();
        }
        ...
        $sql = "UPDATE tbl_add_double_daimond SET user_id='$user_id', user_name='$user_name' WHERE id='$id'";
    }

    $result = mysqli_query($con, $sql);
    if ($result) {
        echo '<script type="text/javascript">alert("Updated successfully.");window.location.href = "view_double_daimond.php";</script>';
    } else {
        echo '<script type="text/javascript">alert("Failed to update.");window.location.href = "view_double_daimond.php";</script>';
    }
}
?>
